...
export default function pesananScreen() {
  ...
  const [isDatePickerVisibleSampai, setDatePickerVisibilitySampai] = useState(false);
  ...
  const getDataPesananBarangFilter = async () =>{
    setPesananBarang([]);
    const orderbarang = [];
    const response = await getAllPesananRequest(ketpesanan, 0, 30, optionfilter, optionfiltertanggal, selectedDate, selectedDateSampai, optionbulan, optionTahun, "");
    response.map((item, i) => (
          orderbarang.push({
            id: item.NoPesanan + i,
            inv: item.NoPesanan,
  ...
  const getDataPesananBarang = async () => {
    ...
    try {
      setPesananBarang([]);
      const orderbarang = [];
      const response = await getAllPesananRequest(ketpesanan, page, 30, optionfilter, optionfiltertanggal, "2025-01-01", "2025-12-31", optionbulan, optionTahun, "");
      ...
      <TouchableOpacity style={{ marginTop: 8, paddingVertical: 10, paddingHorizontal: 15, borderRadius: 15, backgroundColor: '#fff' }} onPress={() => handlePressOrderBarang(item.inv)}>
        <View>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <Text style={{ color: "#585858", fontSize: 12, backgroundColor: item.statusdiset == "pending" ? '#FFD666' : item.statusdiset == "onprocess" ? '#A0C4FF' : item.statusdiset == "rejected" ? '#FFB6B6' : '#9DE0AD', paddingHorizontal: 10, paddingVertical: 3, marginTop: 5, borderRadius: 10 }}>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.containerfilter}>
        <TouchableOpacity style={{ backgroundColor: "#A0C4FF", paddingHorizontal: 5, paddingVertical: 5, borderRadius: 5, marginRight: 8 }} onPress={() => setModalVisible(true)}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          <TouchableOpacity style={[styles.button, styles.activeButton]}>
            <Text style={[styles.buttonText, styles.activeText]}>Semua</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.button}>
            <Text style={styles.buttonText}>Disetujui</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.button}>
            <Text style={styles.buttonText}>Onprocess</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.button}>
            <Text style={styles.buttonText}>Menunggu</Text>
          </TouchableOpacity>
        </ScrollView>
      </View>
      ...
      <View>
        <FlatList
          ...
          onEndReached={() => { getDataPesananBarang() }}
          ...
          <View>
            ...
            <View style={{flexDirection:'row',justifyContent:'space-between',marginTop:10}}>
              <View style={{flex:1,marginRight:5}}>
                <Text style={{fontSize:17,marginBottom:5}}>Tanggal Dari</Text>
                <TouchableOpacity style={[styles.pickerContainer,{height:50,display:'flex',justifyContent:'center',paddingHorizontal:10}]} onPress={showDatePickerDari}>
                  <Text style={{fontSize:17}}>{selectedDate ? selectedDate.toLocaleDateString() : 'Pilih tanggal'}</Text>
                </TouchableOpacity>
                <DateTimePickerModal
              ...
              <View style={{flex:1}}>
                <Text style={{fontSize:17,marginBottom:5}}>Tanggal Sampai</Text>
                 <TouchableOpacity style={[styles.pickerContainer,{height:50,display:'flex',justifyContent:'center',paddingHorizontal:10}]} onPress={showDatePickerSampai}>
                  <Text style={{fontSize:17}}>{selectedDateSampai ? selectedDateSampai.toLocaleDateString() : 'Pilih tanggal'}</Text>
                </TouchableOpacity>
                <DateTimePickerModal
            ...
            <View style={{flexDirection:'row',justifyContent:'space-between'}}>
              <TouchableOpacity
              onPress={() =>{setPage(0),getDataPesananBarangFilter()}}
              style={{flex:1,marginRight:5, height:50,backgroundColor:'#0085c8',justifyContent:'center',alignItems:'center', paddingHorizontal:20, borderRadius: 10,marginTop: 15,}}
            >
              <Text style={[styles.buttonTextrt,{fontSize:17}]}>Proses</Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() => setModalVisible(false)}
              style={{flex:1, height:50,backgroundColor:'#c80035',justifyContent:'center',alignItems:'center',paddingHorizontal:20, borderRadius: 10,marginTop: 15,}}
            >
              <Text style={[styles.buttonTextrt,{fontSize:17}]}>Tutup</Text>
            </TouchableOpacity>
