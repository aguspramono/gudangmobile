...
import { getAllClosingOrderRequest } from "./../func/closingOrderFunc";
import { DateFormat } from "./../func/global/globalFunc";
import { Picker } from "@react-native-picker/picker";
import DateTimePickerModal from "react-native-modal-datetime-picker";
import { router, useFocusEffect } from "expo-router";

function closingOrderScreen() {

    const [closingorder, setClosingOrder] = useState<any[]>([]);
    const [page, setPage] = useState(0);
    const [ketclosingorder, setKetClosingOrder] = useState("");
    ...
    const handlePressOrderBarang = (itemId) => {
        router.navigate({ pathname: "detailClosingOrder", params: { id: itemId } });
    };
    ...
    const fetchData = async (
        ...
        const closingorderitem = [];
        const response = await getAllClosingOrderRequest(
            like,
            limitqueryprev,
            limitquery,
            option,
        ...
        response.map((item, i) =>
            closingorderitem.push({
                id: item.NoClosing + page + i,
                noclosing: item.NoClosing,
                detail: item.jumlahItem + " item closing order",
                tanggal: item.Tgl
            })
        );

        if (setReq === "filter") {
            setClosingOrder(closingorderitem);
        } else {
            setClosingOrder([...closingorder, ...closingorderitem]);
        }
    };
    ...
    const nextPage = async () => {
        ...
        const response = await getAllClosingOrderRequest(ketclosingorder, 0, 0, optionfilter, optionfiltertanggal, tanggaldari, tanggalsampai, optionbulan, optionTahun);
                        ...
                        <View style={{ marginTop: 10 }}>
                            ...
                            <View style={styles.pickerContainer}>
                                ...
                                >
                                    ...
                                    <Picker.Item label="Nama Supplier" value="Nama" />
                                    <Picker.Item label="Kota Supplier" value="Kota" />
                                </Picker>
                            </View>
                            ...
                            <View style={[styles.pickerContainer, { marginTop: 5 }]}>
                                <TextInput
                                    ...
                                    value={ketclosingorder}
                                    onChangeText={setKetClosingOrder}
                                />
                            </View>
                        </View>
...
export default closingOrderScreen;
